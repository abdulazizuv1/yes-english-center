...
function ieltsScore(score, total) {
...
async function loadResults() {
  ...
  try {
      ...
      resultsBody.innerHTML = `
        <tr>
          <td colspan="5" class="no-results">
            ...
            <p>No reading test results available yet.</p>
          </td>
        </tr>
      `;
      return;
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      const band = ieltsScore(score, total);
      ...
      row.addEventListener("click", (e) => {
        if (!e.target.closest(".deleteBtn")) {
          window.location.href = `/pages/mock/result.html?id=${docSnap.id}`;
        }
      });
      ...
      row.querySelector(".deleteBtn").addEventListener("click", async (e) => {
        ...
        if (confirmed) {
          try {
            ...
            await deleteDoc(doc(db, "resultsReading", docSnap.id));
      ...
      <tbody>
        <tr>
          <td colspan="5" style="text-align: center; padding: 40px; color: #64748b;">
            ...
            <p>There was an error loading the reading test results.</p>
