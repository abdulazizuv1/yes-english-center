...
import { useAllUsersResults, useAllUsersAIFeedback, getResultUrl, getScoreDisplay, convertToIELTS, getBandClass, formatType } from '../hooks/useResults';
import { ClipboardList, Headphones, BookOpen, PenTool, FileText, Bot, ArrowLeft, Search, ArrowRight } from 'lucide-react';
...
const TEST_TYPES = [
    ...
    { type: 'aifeedback', icon: Bot, color: '#7c3aed', desc: 'All student AI writing feedback' },
];

function formatTypeAdmin(type) {
    if (type === 'aifeedback') return 'AI Feedback';
    return formatType(type);
}

function AIFeedbackResultsList() {
    const { results, loading } = useAllUsersAIFeedback();
        ...
        <>
            ...
            {filtered.length === 0 ? (
                <div className="utr-empty glass-card">
                    ...
                    <p>{search ? 'No results match your search' : 'No AI feedback submissions yet'}</p>
                </div>
            ) : (
                <div className="utr-results-list">
                    {filtered.map(r => {
                        const overall = r.feedback?.overall ?? '—';
                        const badgeCls = overall !== '—' ? getBandClass(overall) : 'band-writing';
                        ...
                        return (
                            <a
                                key={r.id}
                                href={`/pages/ai-feedback/?id=${r.submissionId}`}
                            ...
                            >
                                ...
                                <span className="utr-result-test">AI Writing Feedback</span>
                                <span className={`utr-result-score ${badgeCls}`}>Band {overall}</span>
                                <span className="utr-result-date">{dateStr}</span>
        ...
        <div className="utr-page page-enter">
            <div className="utr-header">
                ...
                <div>
                    ...
                    <p>{selectedType ? `All ${formatTypeAdmin(selectedType)} results` : 'View all student test results'}</p>
                </div>
            </div>
            ...
            {!selectedType ? (
                ...
                <>
                    ...
                    {selectedType === 'aifeedback'
                        ? <AIFeedbackResultsList />
